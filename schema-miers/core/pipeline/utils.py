...
min_date = current_time_utc - timedelta(days=7)
...
def get_sql_column_names(df_columns):
    ...
    sql_columns = [col.lower().replace(" ", "_").replace('#_', 'number_of_')
                   .replace("(", "").replace(")", "").replace("/", "").replace('-', '_') for col in df_columns]
    return sql_columns
...
def file_exists(file_name):
    cursor.execute(
        "SELECT COUNT(*) FROM UTIL_DB.LOGS.S3_MIERS_FILE_LOG WHERE FILE_NAME = %s AND ENV = %s",
        (file_name, os_env),
    )
...
def insert_into_snowflake_log(folder_name, file_last_modified, file_name):
    cursor.execute(
        """INSERT INTO UTIL_DB.LOGS.S3_MIERS_FILE_LOG (FILE_LAST_MODIFIED_TIMESTAMP, CURRENT_TIME_UTC, ENV, BATCH_ID,
                                                   FOLDER_NAME, FILE_NAME) 
                    VALUES (%s, %s, %s, %s, %s, %s)""",
        # Change the ENV to local if running locally
        (file_last_modified, current_time_utc, os_env, batch_id, folder_name, file_name),
    )
    logging.info(f"File {file_name} logged in Snowflake at UTIL_DB.LOGS.S3_MIERS_FILE_LOG.")
...
def get_ssh_client(host, port, username, password):
    ...
    client.connect(host, port, username, password, allow_agent=False, look_for_keys=False)
    logging.info("Connection to SFTP established.")
    return client
